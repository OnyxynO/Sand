...
const typeConfig: Record<string, { label: string; couleur: string; icone: typeof ClockIcon }> = {
  ...
  SEMAINE_VIDE: {
    label: 'Semaine vide',
    couleur: 'bg-[color:var(--sand-surface-strong)] text-[color:var(--sand-muted)]',
  ...
  SAISIE_SUR_ABSENCE: {
    ...
    couleur: 'bg-[color:var(--sand-accent-soft)] text-[color:var(--sand-accent-strong)]',
    icone: ExclamationTriangleIcon,
  },
};
...
export default function SupervisionPage() {
  ...
  const nbFiltresActifs = [filtreProjet, filtreEquipe, filtreType].filter(Boolean).length;
      ...
      <div className="sand-card rounded-[1.8rem] p-4 space-y-4">
        <div className="flex items-center justify-between">
          ...
          >
            ...
            Filtres
            {nbFiltresActifs > 0 && (
              <span className="bg-[color:var(--sand-accent)] text-white text-xs px-1.5 rounded-full">
                {nbFiltresActifs}
              </span>
            )}
          </button>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {anomalie.date && (
                              <span className="text-xs text-[color:var(--sand-muted)]">
                                {format(new Date(anomalie.date), 'EEEE d MMMM', { locale: fr })}
                              </span>
                            )}
                            {anomalie.semaine && (
                              <span className="text-xs text-[color:var(--sand-muted)]">Semaine {anomalie.semaine}</span>
                            )}
                          </div>
