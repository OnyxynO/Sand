...
- **Codegen — via SDL local (plus d'introspection HTTP)** : `codegen.ts` pointe sur le fichier SDL `frontend/schema.graphql` (commite), genere par `bun run schema` (`php artisan lighthouse:print-schema`). Raison : l'introspection HTTP `http://localhost:8080/graphql` etait bloquee par `max_query_depth = 7` de Lighthouse (une requete d'introspection fait ~13 de profondeur → `Max query depth should be 7 but got 13`). Avec le SDL, `bun run codegen` fonctionne **sans backend qui tourne**. **Workflow apres modif du schema backend** : `bun run schema` puis `bun run codegen`, committer `schema.graphql` + `src/gql/`. Le build prod (`build:docker`) utilise les types commites sans regenerer. ⚠️ le SDL expose `users(first: Int! = 20)` (via `@paginate`) : l'operation `Users` declare `$first: Int = 20` explicitement, sinon la validation codegen echoue.
